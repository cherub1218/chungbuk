                                    	...
                                    	<c:when test="${fn:length(list) != 0}">
	                            			<c:forEach items="${list}" var="list" varStatus="status">
												<tr>
													...
													<td>
														<a style="width : 100%;" href="<c:url value="/bam/businessAnnouncementDetail"/>?bam_anc_idx=${list.bam_anc_idx}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType}&keyword=${scri.keyword}"><c:out value="${list.anc_title}" /></a>
													</td>
													<td>
												...
												<c:choose>
													<c:when test="${member.user_auth == '1' || member.user_auth == '2'}">
														<td class="quickBtn" style="text-align: center;">
															<a href="<c:url value="/bpm/businessPlanApplyList"/>?bam_anc_idx=${list.bam_anc_idx}">접수목록보기</a>
														</td>
													</c:when>
													<c:otherwise>
														<td class="quickBtn" style="text-align: center;">
															<a href="<c:url value="/bpm/businessPlanApply"/>?bam_anc_idx=${list.bam_anc_idx}">접수하기</a>
														</td>
													</c:otherwise>
												</c:choose>	
													</tr>
